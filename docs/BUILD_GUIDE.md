...
Lokus relies on GitHub Actions to keep builds reproducible across platforms. The key workflows live in [`.github/workflows/`](../.github/workflows/):

| Workflow | Purpose | Notes |
| --- | --- | --- |
| [`test.yml`](../.github/workflows/test.yml) | Runs Vitest unit tests on macOS, Windows, and Linux, then performs a smoke Tauri build. | Unit tests can continue-on-error while flaky suites are stabilised. |
| [`build-multi-platform.yml`](../.github/workflows/build-multi-platform.yml) | Produces signed installers for all desktop platforms on every push/PR. | Installs platform dependencies, runs `npm run build`, and invokes `tauri build` with the correct config. |
| [`e2e-tests.yml`](../.github/workflows/e2e-tests.yml) | Executes Playwright specs against the bundled app. | Requires the Playwright browsers cache; failures upload traces for debugging. |
| [`release.yml`](../.github/workflows/release.yml) | Drafts GitHub releases and attaches build artifacts when a tag is published. | Consumes artifacts generated by the build workflow and updates release notes. |

When you add new scripts or environment variables, update the relevant workflow so automation stays in sync with local development. For example, to run a lint step in CI you can extend the `unit-tests` job inside `test.yml`:
...
Secrets such as signing keys are stored in the repository settings (`TAURI_PRIVATE_KEY`, `TAURI_KEY_PASSWORD`). Reference them via the `secrets` context inside workflows.

## 📝 Build Verification

### Quality assurance checklist

- [ ] Application starts without console errors on macOS, Windows, and Linux
- [ ] Core views (Notes, Bases, Graph, Canvas) render sample workspaces correctly
- [ ] File operations (create, import, export) succeed and persist on disk
- [ ] Math, syntax highlighting, and media embeds render as expected
- [ ] Keyboard shortcuts respond according to the shortcut reference
- [ ] Theme switching and window controls behave normally
- [ ] Installer/uninstaller flows complete without warnings
- [ ] Bundle size remains within target thresholds for each platform
...
# Optional: dependency security scans
npm audit
cargo audit --manifest-path=src-tauri/Cargo.toml
```

Document any failing or flaky tests in the pull request so maintainers can triage them. When adding new tooling, ensure the corresponding GitHub Actions workflow is updated so CI matches local expectations.
