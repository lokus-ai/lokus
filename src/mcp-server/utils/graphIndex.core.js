/**
 * Graph Index — pure core (no Node builtins)
 *
 * All the knowledge-graph logic (index building, wiki-link parsing, related-notes
 * / backlink / path / hub / cluster queries) lives here with ZERO filesystem or
 * `path` imports, so it is safe to bundle for the browser/Tauri renderer.
 *
 * IO is delegated to an injected `adapter`, which must implement:
 *   - listMarkdownFiles(): Promise<Array<{ path, relativePath, name }>>
 *       Every `.md` file under the workspace (recursive). `path` is the absolute
 *       path passed back to readFile(); `relativePath`/`name` are precomputed by
 *       the adapter (which knows the platform's path separator).
 *   - readFile(absPath): Promise<string>
 *   - loadCache(): Promise<object|null>   // parsed cache blob, or null if absent
 *   - saveCache(data): Promise<void>      // best-effort persist
 *
 * The Node adapter (graphIndex.js) backs the MCP server; the Tauri adapter
 * (graphIndex.tauri.js) backs the renderer. See those files for wiring.
 */
...
/**
 * GraphIndexCore — platform-agnostic graph engine driven by an IO adapter.
 */
export class GraphIndexCore {
  constructor(workspacePath, adapter) {
  ...
  /**
   * Load graph from cache (via adapter)
   */
  async loadFromCache() {
    return this.adapter.loadCache();
  }

  /**
   * Save graph to cache (via adapter)
   */
  async saveToCache() {
    const data = {
      graph: this.graph,
    ...
    await this.adapter.saveCache(data);
  }
  ...
  async build() {
    ...
    // Scan all markdown files (adapter provides absolute path + relative path + name)
    const files = await this.adapter.listMarkdownFiles();

    // First pass: create nodes
    for (const file of files) {
      const nodeId = this.normalizeNodeId(file.name);
  ...
  async processFile(file) {
    ...
    const sourceId = this.normalizeNodeId(file.name);
