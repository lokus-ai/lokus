...
/**
 * MeetingFAB — floating Lokus logo button to manually start a meeting recording.
 *
 * Expands on hover to show "Record meeting" label.
 * Clicking directly starts recording and opens the right sidebar with MeetingPanel.
 */
export default function MeetingFAB() {
  const [hovered, setHovered] = useState(false);
  const [pressed, setPressed] = useState(false);
  const { state, acceptPrompt } = useMeeting();

  // Only show FAB when a meeting has been detected (prompted state).
  // Hidden by default; hidden again once recording starts (panel takes over).
  if (state !== 'prompted') return null;

  const handleClick = async () => {
    try {
      // Ensure right sidebar is open so MeetingPanel is visible
      const { showRight } = useLayoutStore.getState();
      if (!showRight) useLayoutStore.getState().toggleRight();
      await acceptPrompt();
    } catch (err) {
      console.error('[MeetingFAB] accept prompt error:', err);
    }
  };
  ...
  return (
    <>
      ...
      >
        {/* Lokus logo */}
        <svg className="meeting-fab-icon" width="20" height="20" viewBox="0 0 480 495" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className="meeting-fab-label">Meeting detected</span>

        {/* Subtle pulse ring when idle */}
        <span className="meeting-fab-ring" />
      </button>
      ...
      <style>{`
        .meeting-fab {
          ...
          border: none;
          border-radius: 20px;
          background: var(--color-app-panel, #1e1e1e);
          border: 1px solid var(--color-app-border, rgba(255,255,255,0.08));
          color: var(--color-app-text, #e0e0e0);
          cursor: pointer;
          overflow: hidden;
        ...
        .meeting-fab:hover {
          ...
          gap: 8px;
          background: var(--color-app-accent, #2F81F7);
          color: white;
          border-color: transparent;
          box-shadow: 0 6px 24px rgba(47,129,247,0.35), 0 2px 8px rgba(0,0,0,0.2);
        }

        .meeting-fab[data-pressed] {
        ...
        .meeting-fab:hover .meeting-fab-icon {
        ...
        .meeting-fab:hover .meeting-fab-label {
          max-width: 120px;
          opacity: 1;
        }

        /* Subtle pulse ring — draws attention without being annoying */
        .meeting-fab-ring {
          position: absolute;
          inset: -3px;
          border-radius: 23px;
          border: 1.5px solid var(--color-app-accent, #2F81F7);
          opacity: 0;
          animation: fab-pulse 3s ease-in-out infinite;
          pointer-events: none;
        }

        .meeting-fab:hover .meeting-fab-ring {
          animation: none;
          opacity: 0;
        }
