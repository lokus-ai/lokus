...
/**
 * NotificationWindow — standalone component rendered in a separate Tauri window.
 * Shows a Notion-style notification banner when a meeting is detected.
 * This window is always-on-top, transparent, and positioned at the top-right.
 *
 * Actions:
 *   - Click "Record" → emits lokus:notification-action { action: 'start_recording' }
 *   - Click "Dismiss" → emits lokus:notification-action { action: 'dismiss' }
...
export default function NotificationWindow() {
  ...
  const handleRecord = async (e) => {
    e.stopPropagation();
    await emit('lokus:notification-action', { action: 'start_recording' });
    close();
  };

  const handleDismiss = async (e) => {
    e.stopPropagation();
    await emit('lokus:notification-action', { action: 'dismiss' });
    close();
  };
    ...
    <div style={styles.root}>
      ...
      >
        ...
        <div style={styles.textWrap}>
          ...
          <div style={styles.body}>Click to start recording</div>
        </div>

        {/* Actions */}
        <div style={styles.actions}>
          <button onClick={handleDismiss} style={styles.btnDismiss}>
            Dismiss
          </button>
          <button onClick={handleRecord} style={styles.btnRecord}>
            Record
          </button>
        </div>
      </div>
...
const styles = {
  ...
  title: { fontSize: 13, fontWeight: 600, color: '#fff', lineHeight: 1.3 },
  body: { fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.3, marginTop: 2 },
