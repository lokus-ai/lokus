 ...
 * Wires plugin-registered completion providers to ProseMirror editor
 */
...
export function createPluginCompletionPlugin(view) {
  return createSuggestionPlugin({
    ...
    command: ({ editor: editorView, range, props }) => {
      ...
      // Delete the trigger character and query, then insert completion text
      const { state } = editorView;
      const tr = state.tr.delete(range.from, range.to);
      const insertPos = tr.mapping.map(range.from);
      tr.insertText(insertText, insertPos);
      tr.scrollIntoView();
    ...
    render: () => {
      ...
      return {
        onStart: (props) => {
          ...
          component = new ReactPopup(PluginCompletionList, props);
...
export default createPluginCompletionPlugin;
