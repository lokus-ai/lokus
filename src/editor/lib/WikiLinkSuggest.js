...
// ---------------------------------------------------------------------------
// Helpers for PM transactions (replacing editor.chain().focus()... patterns)
// ---------------------------------------------------------------------------

/**
 * Focus the view's DOM, delete a range, and optionally set cursor position.
 */
function focusAndDelete(view, from, to) {
...
/**
 * Focus the view, delete a range, then insert content (a PM JSON node spec or string).
 */
function focusDeleteAndInsert(view, from, to, content) {
  view.dom.focus({ preventScroll: true })
...
/**
 * Insert ]] auto-pair at cursor if not already present, then move cursor back.
 */
function autoInsertClosingBrackets(view, pos) {
  try {
    const docSize = view.state.doc.content.size
    const safePos = Math.max(0, Math.min(docSize, pos))
    const endPos = Math.min(docSize, safePos + 2)
    const next = view.state.doc.textBetween(safePos, endPos)
    if (next !== ']]') {
      // Insert ]] then move cursor back before them
      const tr = view.state.tr.insertText(']]', safePos)
      tr.setSelection(TextSelection.create(tr.doc, safePos))
      view.dispatch(tr)
    } else {
      // Just move cursor to before the existing ]]
      const tr = view.state.tr.setSelection(TextSelection.create(view.state.doc, safePos))
      view.dispatch(tr)
    }
  } catch {}
}
...
// ---------------------------------------------------------------------------
// Shared place() helper for WikiLink suggestion popups
// ---------------------------------------------------------------------------

function createPlaceFn(containerRef) {
  return (rect) => {
...
// ---------------------------------------------------------------------------
// Shared render lifecycle factory for WikiLink suggestion popups
// ---------------------------------------------------------------------------

function createWikiLinkRender(extraProps = {}) {
  return () => {
    let component
    let container
    const place = createPlaceFn(() => container)

    return {
      onStart: (props) => {
        const mergedProps = { ...props, ...extraProps }

        // Handle auto-pair for wiki links (not canvas)
        if (extraProps._autoInsertClosing) {
          const view = props.editor
          try {
            const range = props.range
            const textBefore = view.state.doc.textBetween(Math.max(0, range.from - 2), range.from)
            const isWikiLink = textBefore === '[[' // Must be exactly [[, not ![

            if (isWikiLink) {
              const pos = Math.max(0, Math.min(view.state.doc.content.size, (props.range?.to ?? view.state.selection.to)))
              autoInsertClosingBrackets(view, pos)
            }
          } catch {}
        }

        if (extraProps._autoInsertClosingImage) {
          const view = props.editor
          try {
        ...
        component = new ReactPopup(WikiLinkList, mergedProps)
      ...
      onUpdate: (props) => {
        const mergedProps = { ...props, ...extraProps }
        component.updateProps(mergedProps)
        if (props.clientRect) place(props.clientRect())
      },
      onKeyDown: (props) => {
...
// ---------------------------------------------------------------------------
// Main export: creates all WikiLink suggestion plugins
// ---------------------------------------------------------------------------

/**
 * Create all WikiLink-related suggestion plugins.
 *
 * @param {import('prosemirror-view').EditorView} view - The ProseMirror EditorView
 * @returns {import('prosemirror-state').Plugin[]} Array of 5 plugins
 */
export function createWikiLinkSuggestPlugins(view) {
  return [
    ...
    new Plugin({
      ...
      props: {
        handlePaste: (pasteView, event, slice) => {
          ...
          focusDeleteAndInsert(pasteView, from, to, {
            type: 'wikiLink',
            attrs: {
              id,
              target: pastedText,
    ...
    createSuggestionPlugin({
      ...
      items: async ({ query, editor: editorView }) => {
      ...
      command: async ({ editor: editorView, range, props }) => {
        if (props.type === 'block') {
          ...
          if (needsExplicitId) {
            ...
            const blockIdMgr = (await import('../../core/blocks/block-id-manager.js')).default

            // Generate new random ID
            const newBlockId = blockIdMgr.generateId()
              ...
              queueBlockIdWrite(fileEntry.path, props.line, newBlockId)
                .then((success) => {
                  if (success) {
                    ...
                    blockIdMgr.invalidateFile(fileEntry.path)
                  }
                })
                .catch(err => {
                })
          ...
          try {
            ...
            const id = globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`
            focusDeleteAndInsert(editorView, absoluteOpenPos, deleteEnd, {
              type: 'wikiLink',
              attrs: {
                id,
                target: wikiLinkText,
      ...
      render: createWikiLinkRender({ _autoInsertClosing: true }),
    }),
    ...
    createSuggestionPlugin({
      ...
      render: createWikiLinkRender({ isImageMode: true, _autoInsertClosingImage: true }),
    }),
    ...
    createSuggestionPlugin({
      ...
      command: ({ editor: editorView, range, props }) => {
        const from = range?.from ?? editorView.state.selection.from
        const to = range?.to ?? editorView.state.selection.to
        const blockId = props.blockId

        // Delete the ^ and query, insert blockid]]
        try {
          focusDeleteAndInsert(editorView, from - 1, to, `^${blockId}]]`)
        } catch (e) {
          // Silent fail
        }
      },
...
export default createWikiLinkSuggestPlugins
