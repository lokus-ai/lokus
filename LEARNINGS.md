...
- (synthesizer) [architecture] Multi-provider calendar backends (Google, CalDAV, iCloud, iCal) require a dedup engine at the sync layer to handle event duplication across providers; fingerprinting + storage dedup in `sync/` is the stable pattern.
- (synthesizer) [constraint] Calendar events and local schedule blocks are architecturally disjoint (separate Contexts, separate backends); unifying them requires a merge layer, not retrofitting into one context.
- (synthesizer) [pattern] OAuth token storage must support both OS keyring (release) and file fallback (debug); PKCE with local redirect server on port 9080 is the validated pattern for Tauri.
- (synthesizer) [gotcha] Frontend hooks (useCalendar, useCalendarEvents, useDeduplicatedEvents) decoupled from actual component usage is a recon anti-pattern; dead code in UI layer signals architectural mismatch between frontend intent and backend.
